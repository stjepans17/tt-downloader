...
export default function ErrorToast({onClose}: ErrorToastComponentProps) {
  
  function handleClose() {
    if(onClose) {
      // false meaning not to show the toast
      onClose(false);
    }
  };
  ...
  return (
    <div id="toast-danger" className="flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow-sm dark:text-gray-400 dark:bg-gray-800" role="alert">
      ...
      <div className="ms-3 text-sm font-normal">Error while processing the video.</div>
      <button type="button" onClick={handleClose} className="ms-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700 hover:cursor-pointer" data-dismiss-target="#toast-danger" aria-label="Close">
